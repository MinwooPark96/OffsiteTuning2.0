        ...
        for step, dataloaders in enumerate(dataloader_zipped):
            ...
            if "T5" in config.get("target_model","model_base"):
                ...
                for idx,(source_dataset,target_dataset) in enumerate(dataloaders):
                    ...
                    with torch.no_grad():
                        source_embedding = source_encoder.bert.embeddings(input_ids = source_dataset['inputx'],attention_mask=source_dataset['mask'])[:,100:]
                        ...
                        target_embedding = target_encoder.roberta.embeddings(input_ids = target_dataset['inputx'],attention_mask=target_dataset['mask'])[:,100:]
